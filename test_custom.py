...
if __name__ == '__main__':
    ...
    for i, data in enumerate(dataset):
        ...
        Image.fromarray(((visuals['fake'][0].cpu() + 1) * 2.0 / 255.0).numpy().astype(np.uint8)).save(f"results/testFakeOutput_{i}.jpg")
        Image.fromarray(((visuals['real'][0].cpu() + 1) * 2.0 / 255.0),numpy().astype(np.uint8)).save(f"results/testRealOutput_{i}.jpg")
    ...
    advGAN = AdvGAN_Attack("cuda", model.netG, 3)

    #advGAN.train(range(10000), 10)
    print('Running attack')
    advGAN.train(dataloader, 10)
